...
const AuctionDetails = () => {
  ...
  const [markers, setMarkers] = useState([
    { lat: 23.8103, lng: 90.4125, title: "Dhaka" }, // initial marker
  ]);
  ...
  function AddMarkerOnClick() {
    useMapEvents({
      click(e) {
        ...
        setMarkers((prev) => [
          ...prev,
          { lat, lng, title: `Marker ${prev.length + 1}` },
        ]);
      },
    });
    return null;
  ...
  const scroll = (direction) => {
    if (containerRef.current) {
      const scrollAmount = 300; // adjust depending on card width
      if (direction === "left") {
        ...
        <div className="justify-center items-center !px-100">
          <div className="!mt-5 text-sm">
            ...
            <div className="flex justify-between gap-90 !py-2">
              ...
              <div>52 Preston Rd. Inglewood, Maine, 6786</div>
            </div>
            <div className="flex justify-between gap-90 !py-2">
              <div>
                <h3>Unloading Address:</h3>
              </div>
              <div>19 Thornridge C. Shiloh, Hawaii, 4567</div>
            </div>
            <div className="flex justify-between gap-90 !py-2">
              <div>
                <h3>Distance:</h3>
            ...
            <div className="flex justify-between gap-90 !py-2">
              ...
              <div>
                The Samsung 32 Y1 Y Series 32-Inch Android TV is Give your eyes
                pleasure with the 16M Display colors. You can connect anything
                with the Samsung TV Y series, very useful connections.
              </div>
            </div>
            <div className="flex justify-between gap-90">
              ...
              <div>12/08/24 at 10 PM</div>
            </div>
            <div className="flex justify-between gap-90">
              <div>
                <h3>Service Date:</h3>
              </div>
              <div>12/08/24 at 4 PM</div>
            </div>
            <div className="flex justify-between gap-90">
              <div>
                <h3>Status:</h3>
          ...
          <div>
            ...
            <div className="relative w-full flex items-center justify-center py-10">
              ...
              <div className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2 z-10">
                <button
                  onClick={() => scroll("left")}
                  className="bg-white !p-4 shadow rounded-full"
                >
              ...
              <div
                ref={containerRef}
                className="flex gap-6 overflow-hidden px-10 scroll-smooth scrollbar-hide"
              >
                {cards.map((card) => (
                  <div
                    key={card.id}
                    className="w-58 h-60 bg-gray-100 shadow rounded-xl flex flex-col items-center justify-center p-4 text-center flex-shrink-0"
                  >
                    <img
                      src={card.image}
                      alt={card.title}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-center text-sm py-2">
                        <p>Bid :</p>
                        <p className="text-gray-400" style={{ color: "blue" }}>
                          {card.description}
                        </p>
                      </div>
                    </div>
          ...
          <div className="!mt-5">
            <div style={{ height: "500px" }}>
              ...
              >
                ...
                {/* Enable clicking on map to add markers */}
                <AddMarkerOnClick />
              </MapContainer>
            </div>
          </div>
